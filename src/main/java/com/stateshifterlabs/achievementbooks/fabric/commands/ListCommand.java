...
public class ListCommand {
    public static int list(CommandContext<ServerCommandSource> c, Books books) throws CommandSyntaxException {
        ...
        String player = source.getPlayerOrThrow().getName().getContent();
        source.sendFeedback(new LiteralTextContent("Currently loaded Achivement Books:\n"), false);
        for (Book book : books) {
            String bookIdRaw = AchievementBooks.MODID + ":" + book.itemName();
            LiteralTextContent bookId = new LiteralTextContent("( " + bookIdRaw + " )");
            ...
            Text toSend = new LiteralTextContent(txt).append(bookId);
