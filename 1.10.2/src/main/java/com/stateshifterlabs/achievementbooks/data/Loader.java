			...
			try {
				...
				try {
					...
					throw new JsonParseError(
							"There is an error in the book config. Use http://jsonlint.com/ to find it", conf);
				}
		...
		for (Book book : books) {
			if (items.containsKey(book.itemName())) {
				...
				if (book.isCraftable()) {
					...
					GameRegistry.addRecipe(itemStack, "AB", 'A', Items.BOOK, 'B', Item.getByNameOrId(book.material()));
				}
